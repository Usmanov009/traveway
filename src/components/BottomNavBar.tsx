...
import { Search, Flame, Ticket, User } from 'lucide-react';
...
export const BottomNavBar: React.FC<BottomNavBarProps> = ({
  ...
  return (
    <nav
      ...
      className={`shrink-0 z-40 border-t px-4 py-2 flex items-center justify-around select-none transition-colors backdrop-blur-md ${
    ...
    >
      ...
      >
        ...
        <div className="relative flex items-center justify-center">
          <Ticket size={20} className={activeTab === 'trips' ? 'stroke-[2.6]' : 'stroke-2'} />
          {activeTab === 'trips' && (
            <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1.5 h-1.5 rounded-full bg-[#ff6600]"></span>
          )}
        </div>
        <span className={`text-[10px] tracking-tight ${activeTab === 'trips' ? 'font-black' : 'font-medium'}`}>
          {language === 'uz' ? "Bronlarim" : "Мои брони"}
        </span>
      </button>
